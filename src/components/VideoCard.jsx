...
const VideoCard = ({displayData, setDeleteVideoResponse, insideCategory}) => {
  ...
  const {caption, link} = displayData //destructured
  const today = new Date()
  // console.log(today);
    //creating timestamp by creating object
  ...
  return (
    <>
      ...
      <Card.Body>
        <Card.Text className='d-flex justify-content-between'>
          <p className='pt-3'>{displayData?.caption}</p>
    ...
    <Modal size='lg' centered show={show} onHide={handleClose}>
        ...
        <Modal.Body>
        <iframe width="100%" height="350" src={`${displayData?.link}?autoplay=1`} title="caption" 
